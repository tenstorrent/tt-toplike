...
- **`src/backend/`** — telemetry sources behind the `TelemetryBackend` trait: `sysfs` (hwmon), `json` (`tt-smi -s`), `hybrid` (sysfs + background JSON), `host` (CPU/RAM/GPU/ANE), `mock`, `ws` (remote), and `luwen`. **Safety note:** `luwen` telemetry is kernel-mediated (tt-kmd ioctl + mmap) and mostly passive reads — but Wormhole/Grayskull send one ARC mailbox message on first read, and Luwen/UMD resource arbitration is unresolved upstream (DEVINFRA-4445), so `factory.rs` keeps it out of auto-detect *and* out of the live `b` cycle. It is reachable only by launching with `--backend luwen`.
