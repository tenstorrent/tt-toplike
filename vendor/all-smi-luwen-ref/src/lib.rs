...
use all_smi_luwen_if::{FnDriver, FnOptions};
use all_smi_ttkmd_if::{PciError, PossibleTlbAllocation};
...
pub use all_smi_ttkmd_if::{DmaBuffer, DmaConfig, PciDevice, Tlb};
...
impl ExtendedPciDevice {
    pub fn open(pci_interface: usize) -> Result<ExtendedPciDeviceWrapper, all_smi_ttkmd_if::PciError> {
        let device = PciDevice::open(pci_interface)?;

        let (grid_size_x, grid_size_y) = match device.arch {
            all_smi_luwen_core::Arch::Grayskull => (13, 12),
            all_smi_luwen_core::Arch::Wormhole => (10, 12),
            all_smi_luwen_core::Arch::Blackhole => (17, 12),
        };
        ...
        if device.arch != Arch::Grayskull && device.driver_version >= 2 {
            ...
            default_tlb = PossibleTlbAllocation::Hardcoded(match device.arch {
                all_smi_luwen_core::Arch::Grayskull | all_smi_luwen_core::Arch::Wormhole => 184,
                all_smi_luwen_core::Arch::Blackhole => 190,
            });
        }
    ...
    match op {
        FnOptions::Driver(op) => match op {
            FnDriver::DeviceInfo(info) => {
                ...
                if !info.is_null() {
                    unsafe {
                        *info = Some(all_smi_luwen_if::DeviceInfo {
        ...
        FnOptions::Axi(op) => match op {
            all_smi_luwen_if::FnAxi::Read { addr, data, len } => {
                if len > 0 {
                    if len <= 4 {
            ...
            all_smi_luwen_if::FnAxi::Write { addr, data, len } => {
                if len > 0 {
        ...
        FnOptions::Noc(op) => match op {
            ...
            all_smi_luwen_if::FnNoc::Broadcast {
                noc_id,
                addr,
                data,
                len,
                ...
                let (x_start, y_start) = match writer.device.arch {
                    all_smi_luwen_core::Arch::Grayskull => (0, 0),
                    all_smi_luwen_core::Arch::Wormhole => (1, 0),
                    all_smi_luwen_core::Arch::Blackhole => (0, 1),
                };
            ...
            all_smi_luwen_if::FnNoc::Multicast {
                noc_id,
                start_x,
                start_y,
                end_x,
...
pub fn open(interface_id: usize) -> Result<all_smi_luwen_if::chip::Chip, LuwenError> {
    let ud = ExtendedPciDevice::open(interface_id)?;

    let arch = ud.borrow().device.arch;

    Ok(all_smi_luwen_if::chip::Chip::open(
        arch,
        all_smi_luwen_if::CallbackStorage::new(comms_callback, ud.clone()),
    )?)
}
