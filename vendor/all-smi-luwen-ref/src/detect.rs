...
use all_smi_ttkmd_if::PciDevice;
...
impl InProgressDetect {
    pub fn detect<E>(
        ...
        let mut chips = all_smi_luwen_if::detect_chips(self.chips, &mut callback, self.options);
...
pub fn detect_chips_options_notui(
    ...
    let init_callback = |status: ChipDetectState| {
        match status.call {
            ...
            all_smi_luwen_if::chip::CallReason::ChipInitCompleted(status) => {
    ...
    let chips = match detect.detect(init_callback) {
        Err(InitError::CallbackError(err)) => {
            ...
            return Err(all_smi_luwen_if::error::PlatformError::Generic(
                "Hit error from status callback".to_string(),
                all_smi_luwen_if::error::BtWrapper::capture(),
            ))?;
        }
    ...
    for chip in chips {
        output.push(
            chip.init(&mut |_| Ok::<(), Infallible>(()))
                .map_err(Into::<all_smi_luwen_if::error::PlatformError>::into)?,
        );
    }
