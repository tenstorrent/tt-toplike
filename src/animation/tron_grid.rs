//! TRON Grid Mode - Neon topology with glowing power lines
//!
//! Inspired by TRON (1982) - neon grids, glowing geometric patterns, flowing energy.
//!
//! Visual elements:
//! - Chip topology as glowing neon grid
//! - Tensix cores as pulsing nodes at grid intersections
//! - Memory channels as perimeter nodes
//! - Data flow as animated power lines between devices
//! - ARC health affects grid stability (flicker when failed)
//!
//! Randomization: Grid pattern, flow speed, color scheme variations

use crate::animation::{AdaptiveBaseline, ansi_color, arc_health_color, hsv_to_rgb, temp_to_hue, value_to_block_char};
...
impl TronGrid {
    /// Create new TRON grid mode with random parameters
    ...
    fn create_title_line<B: TelemetryBackend>(&self, backend: &B) -> Line<'static> {
        ...
        spans.push(Span::styled(
            format!(" TRON GRID MODE ({:?}) ", self.grid_style),
            Style::default()
                .fg(self.color_scheme.bright_color(self.frame))
    ...
    /// Render a single device with DDR/memory visualization
    fn render_device_grid<B: TelemetryBackend>(
        ...
        let mut lines = Vec::new();
        let chars = self.grid_style.chars();
        ...
        let base_color = if arc_healthy {
            self.color_scheme.base_color(self.frame)
        } else {
            // Solid red when ARC failed (no blinking)
            Color::Rgb(255, 80, 80)
        };

        let bright_color = if power_change > 0.2 || current_change > 0.2 {
            self.color_scheme.bright_color(self.frame)
        } else {
            base_color
        };
        ...
        top_line.push(Span::styled(
            title,
            Style::default().fg(bright_color).add_modifier(Modifier::BOLD),
        ));
        top_line.push(Span::styled(
            chars.horizontal.to_string().repeat(content_width.saturating_sub(title_len)),
        ...
        // DDR Channels visualization
        lines.push(self.render_ddr_channels(
            backend,
            device,
            content_width,
            &chars,
        ...
        lines.push(self.render_l2_cache(
            backend,
            device,
            content_width,
            &chars,
        ...
        // L1 SRAM visualization (compressed Tensix grid)
        lines.extend(self.render_l1_sram(
            backend,
            device,
            content_width,
            &chars,
        ...
        // Stats line
        let stats = format!(
            " Power: {:.1}W │ Temp: {:.0}°C │ Current: {:.1}A ",
            power, temp, current
        );
    ...
    /// Render DDR channels with training status
    fn render_ddr_channels<B: TelemetryBackend>(
        &self,
        backend: &B,
        device: &crate::models::Device,
        content_width: usize,
        ...
        // Get DDR channel count based on architecture
        ...
        // Add utilization blocks based on current draw
        let utilization = (current / 100.0).min(1.0);  // Normalize to 0-1
        let num_blocks = (utilization * 8.0) as usize;

        spans.push(Span::raw(" │ "));
        for i in 0..8 {
            let block_char = if i < num_blocks {
                value_to_block_char(1.0 - (i as f32 / 8.0))
            } else {
                '·'
            };
            let block_color = if i < num_blocks {
                Color::Rgb(255, 180, 100)  // Orange for activity
            } else {
                Color::Rgb(80, 80, 100)
            };
            spans.push(Span::styled(
        ...
        let used_width = 7 + num_channels * 2 + 3 + 8;  // Approximate
        spans.push(Span::raw(" ".repeat(content_width.saturating_sub(used_width))));
        spans.push(Span::styled(
            chars.vertical.to_string(),
            Style::default().fg(base_color),
        ));
    ...
    /// Render L2 cache visualization
    fn render_l2_cache<B: TelemetryBackend>(
        &self,
        _backend: &B,
        ...
        // Show 8 L2 cache banks (typical architecture)
        let l2_activity = current_change.max(0.0).min(1.0);
        for i in 0..8 {
            // Create wave pattern across banks
            let bank_phase = (i as f32 * 0.5 + self.frame as f32 * 0.1).sin() * 0.3;
            let bank_activity = (l2_activity + bank_phase).max(0.0).min(1.0);

            let bank_char = value_to_block_char(bank_activity);
            let bank_color = if bank_activity > 0.7 {
                Color::Rgb(255, 200, 80)
            } else if bank_activity > 0.4 {
    ...
    /// Render L1 SRAM (compressed Tensix grid)
    fn render_l1_sram<B: TelemetryBackend>(
        &self,
        _backend: &B,
        device: &crate::models::Device,
        content_width: usize,
        ...
        for display_row in 0..display_rows {
            ...
            // Render compressed cores with power-responsive activity
            for col in 0..grid_cols {
                // Calculate core activity with wave patterns
                let wave = (actual_row as f32 * 0.5 + col as f32 * 0.5 + self.frame as f32 * 0.15).sin() * 0.3;
                let core_activity = (power_change + wave).max(0.0).min(1.5);

                let core_char = if core_activity > 1.0 {
                    '█'
                } else if core_activity > 0.7 {
                    '▓'
                } else if core_activity > 0.4 {
                    '▒'
                } else if core_activity > 0.2 {
                    '░'
                } else {
                    '·'
                };
    ...
    fn create_legend_line(&self) -> Line<'static> {
        Line::from(vec![
            ...
            Span::raw(" Trained "),
            Span::styled("◐", Style::default().fg(Color::Rgb(80, 220, 220))),
            Span::raw(" Training "),
            Span::styled("○", Style::default().fg(Color::Rgb(100, 100, 120))),
            Span::raw(" Idle "),
            Span::styled("✗", Style::default().fg(Color::Rgb(255, 100, 100))),
            Span::raw(" Error  │  "),
            Span::styled("v", Style::default().fg(Color::Rgb(80, 220, 200)).add_modifier(Modifier::BOLD)),
            Span::raw(" randomize"),
        ])
    }
...
mod tests {
    ...
    fn test_tron_grid_creation() {
        ...
        assert_eq!(grid.mode_name(), "TRON Grid");
